...
The manually runnable **Rebuild allowlisted library documentation** workflow accepts only `main`-branch workflow-dispatch inputs. It validates repository, tag, version, and commit before querying fixed GitHub API paths for the matching public release, tag, and `docs/library.yml`. It never clones, renders, executes, or imports upstream content. It writes only portal-derived release metadata and fixed-template cards, then builds and deploys that exact portal commit.
