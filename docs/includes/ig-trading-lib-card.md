<article class="portal-card" data-library="ig-trading-lib">
  <div class="portal-card-heading">
    <p class="portal-status">documentation available · v3.0.0</p>
    <h2>IG Trading Library</h2>
    <p class="portal-summary">Safe, typed synchronous and asynchronous IG REST and streaming clients.</p>
  </div>
  <ul class="portal-points">
    <li>Immutable release documentation</li>
    <li>Latest stable documentation alias</li>
    <li>Safety-first trading guidance</li>
  </ul>
  <p class="portal-install"><code>pip install ig-trading-lib</code></p>
  <div class="portal-card-actions" aria-label="IG Trading Library links">
    <a href="https://evgesha9400.github.io/ig-trading-lib/3.0.0/">Documentation v3.0.0</a>
    <a href="https://evgesha9400.github.io/ig-trading-lib/latest/">Latest documentation</a>
    <a href="https://github.com/evgesha9400/ig-trading-lib">Source code</a>
    <a href="https://pypi.org/project/ig-trading-lib/">Package on PyPI</a>
  </div>
</article>
